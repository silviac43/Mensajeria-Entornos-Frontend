...
export default function HistorialPedidosOperador() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      setLoading(true);
      try {
        const datosOperador = await obtenerInformacionOperador();
        setOperador(datosOperador); 
        
        if (!datosOperador?.empresaMensajeria?.id) {
          setError('No se pudo obtener la empresa del operador');
          setLoading(false);
          return;
        }
        await cargarHistorialPorEmpresa(datosOperador.empresaMensajeria.id);
      } catch (err) {
      ...
      <>
        <div className="container py-4" style={{ minHeight: 'calc(100vh - 120px)' }}>
          ...
          <p className="text-muted mb-0 fw-semibold">
            ...
            Empresa: <span className="fw-semibold">{operador?.empresaMensajeria?.nombre || 'Cargando...'}</span>
          </p>
      ...
      <div className="container py-4" style={{ minHeight: 'calc(100vh - 120px)' }}>
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            ...
            <p className="text-muted mb-0 fw-semibold">
              <i className="bi bi-buildings me-1" style={{ color: '#ff6600' }}></i>
              Empresa: <span className="fw-semibold">{operador?.empresaMensajeria?.nombre}</span>
            </p>
          </div>
        ...
        {historialFiltrado.length === 0 ? (
          <div className="text-center py-5">
            ...
            <h4 className="text-muted mt-3">
              {historial.length === 0 ? 'No hay cambios registrados para su empresa' : 'No se encontraron resultados'}
            </h4>
            <p className="text-muted">
              {historial.length === 0 
                ? 'Aún no se han registrado cambios en los pedidos de su empresa.' 
